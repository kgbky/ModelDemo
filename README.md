...
2.开闭原则(扩展开放修改关闭)
编写代码时需要注意该原则，需求总是在不断变化，且大部分需求都是扩展或新增功能
...
#### 12.Decorator(装饰)模式
装饰边框与被装饰物的一致性，添加装饰边框来增加功能

* 把边框也看成被装饰物，形成递归结构，方便扩展功能

* 添加装饰物可以添加许多新功能

#### 13.Visitor(访客)模式
数据 和 数据处理 被分开。访客负责数据处理的工作
...
Facade角色封装了复杂的具体操作，系统外部只调用一个api，且系统内部并不使用Facade角色

减少了复杂系统与调用该系统的类的耦合。超大系统内部使用该模式能提高系统的可维护性

#### 16.Mediator（仲裁者、中介者）模式
...
* narrow interface(窄接口)：暴露的内容很少(public修饰)
* 通过宽窄api使代码更符合迪米特原则
...
#### 21.Proxy（代理）模式
让Proxy负责完成工作，当Proxy不能胜任时，会委托回本体。

* 需要延迟操作(绝大部分是耗时或耗内存的操作)时，可使用代理模式
* 代理模式有很多变体，每种变体解决的问题不同

#### 22.Command（命令）模式
GUI编程中，经常需要与"事件（按下等）"打交道，把事件用类表示，就是Command模式。

*延伸：有时，用对象表示一些现实中不存在的“物”会带来很好的效果
